...
walk('./src', (err, files) => {
  ...
  files.forEach(file => {
    ...
    content = content.replace(/bg-\[#050505\]/g, 'bg-[#EFEFEF] dark:bg-[#050505]');
    content = content.replace(/bg-zinc-900/g, 'bg-[#F7F7F7] dark:bg-zinc-900');
    content = content.replace(/bg-zinc-950/g, 'bg-[#EFEFEF] dark:bg-zinc-950');
    ...
    content = content.replace(/border-white\/10/g, 'border-[#D0D0D0] dark:border-white/10');
    content = content.replace(/border-white\/20/g, 'border-[#C0C0C0] dark:border-white/20');
    content = content.replace(/border-white\/30/g, 'border-[#B0B0B0] dark:border-white/30');

    // Overlay bgs
    content = content.replace(/bg-white\/5 /g, 'bg-[#F0F0F0] dark:bg-white/5 ');
    content = content.replace(/bg-white\/5"/g, 'bg-[#F0F0F0] dark:bg-white/5"');
    content = content.replace(/bg-white\/10 /g, 'bg-[#E5E5E5] dark:bg-white/10 ');
    
    // Misc
    content = content.replace(/bg-black\/80/g, 'bg-white/80 dark:bg-black/80');
    content = content.replace(/bg-black\/30/g, 'bg-[#EFEFEF]/80 dark:bg-black/30');
    content = content.replace(/bg-black/g, 'bg-[#FFFFFF] dark:bg-black');
